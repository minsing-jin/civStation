...
<div align="center">
  <img src="docs/readme_resources/architecture_abstraction.png" alt="HITL 주문이 background strategy agent로 들어가고, strategy와 order가 main process action agent로 내려가며, context가 다시 위로 올라가는 간단한 아키텍처 다이어그램" width="100%" />
</div>
...
이 모드는 스크린샷, 픽셀 라우팅, pyautogui 액션을 사용하지 않습니다.
CivStation은 upstream 서버를 stdio JSON-RPC subprocess로 시작하고 Python `mcp` SDK를 통해 통신합니다.
...
Civilization VI의 `AppOptions.txt`를 열고 다음 값을 설정합니다:
...
이 명령은 stdio MCP 서버를 시작한 뒤 JSON-RPC 입력을 기다리는 상태가 정상입니다.
`Ctrl+C`로 중지하세요. 실제 CivStation 실행 중에는 CivStation이 이 서버를 자동으로 시작합니다.
...
`--civ6-mcp-path`는 기본적으로 `$CIV6_MCP_PATH`를 사용하고, 없으면 `~/civ6-mcp`를 사용합니다.
`--civ6-mcp-launcher uv`는 다음 명령을 실행합니다:

```bash
uv run --directory <civ6-mcp-path> civ-mcp
```

upstream checkout의 virtual environment를 직접 활성화해서 쓰고 싶다면 `--civ6-mcp-launcher python`을 사용하세요.
이 경우 CivStation은 `python -m civ_mcp`를 실행합니다.
