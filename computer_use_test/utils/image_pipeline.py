...
def process_image(
    ...
    if config.ui_filter_mode != "none":
        from computer_use_test.utils.ui_benchmarking import apply_ui_filter

        img = apply_ui_filter(img, config.ui_filter_mode)

    # 2. Colour policy
    if config.color_policy != "preserve":
        from computer_use_test.utils.ui_benchmarking import apply_color_policy
    ...
    if config.encode_mode != "none":
        from computer_use_test.utils.ui_benchmarking import simulate_transport_encoding
