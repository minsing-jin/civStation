...
2. 특수지구/불가사의/건물 배치 화면:
   ...
   - 현재 보유 골드와 파란 타일에 표시된 구매 골드를 비교해, 실제로 지불 가능한 경우에만 파란 타일을 선택해.
   - 파란색 타일은 인접 보너스, 지형 시너지, 상위 전략 목표를 함께 봤을 때 초록 타일보다 확실히 유리할 때만 선택해.
   - 최적 타일 1곳을 클릭하고 아직 task_status는 "in_progress"로 둬.
   - 파란 타일을 눌러 구매가 끝났는데도 배치 화면이 유지되면 같은 타일을 다시 클릭해 실제 배치를 이어가.
