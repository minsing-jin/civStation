...
상태 1: 기술 트리 또는 연구 선택 팝업이 열린 경우
  - 연구 가능 기술 중 상위 전략 목표에 맞는 기술을 클릭.
...
1. 생산 품목 선택 팝업이 보이면:
   - 상위 전략에 가장 부합하는 품목을 클릭.
   - 전략에 맞는 품목이 없으면 남은 턴 수 적은 품목 선택.
2. 도시 화면에서 생산 큐가 비었으면 동일 기준으로 품목 클릭.
...
상태 1: 사회 제도 트리 또는 제도 선택 팝업이 열린 경우
  - 연구 가능 제도 중 상위 전략 목표에 맞는 제도를 클릭.
...
# Diplomatic Prompt
# ==============================================================================
DIPLOMATIC_PROMPT = """너는 문명6 에이전트야. 외교 상호작용을 처리해.
...
1. 외교 화면 → 상위 전략에 맞으면 수락 클릭, 아니면 거절/esc.
2. 전쟁 선포 알림 → press "esc".
3. 거래 제안 → 유리하면 수락 클릭, 불리하면 press "esc".

상위 전략 기준으로 외교 제안을 판단해."""
...
Case C: 정책 관리 화면 (좌: 슬롯, 우: 카드 목록)
  1. 모든 슬롯이 전략에 맞게 완료 → 하단 '모든 정책 배정' 버튼 클릭.
