...
2. 특수지구/불가사의/건물 배치 화면:
   ...
   - 구매형 타일은 타일 본체가 아니라 골드와 숫자가 보이는 구매 버튼/배지를 먼저 클릭해.
   - 초록 타일은 타일 본체를 직접 클릭하고, 구매형 타일은 구매 뒤 배치 화면이 유지되면
     같은 타일 본체를 다시 클릭해 실제 배치를 이어가.
...
상황 2: 총독 능력 진급 팝업 ([확정] 비활성화/검은색)
  - 이미 진급을 결정했다면 반드시 스킬 1개를 먼저 고른다.
  - [확정]이 비활성/검은색이면 절대 클릭하지 마.
  - 상위 전략과 현재 상태에 맞는 스킬 버튼 클릭. task_status="in_progress".

상황 3: 총독 능력 진급 팝업 ([확정] 활성화/초록색)
  - [확정] 버튼 클릭 → task_status="complete".

상황 4: 도시 선택 지도 (좌상단 "총독 배정" 텍스트)
  - 왼쪽 팝업/목록에 보이는 도시 단서를 기준으로 상위 전략에 맞는 도시를 먼저 클릭해.
  - [배정] 버튼이 검은색/비활성이면 절대 클릭하지 마.
  - 도시 선택은 task_status="in_progress".
