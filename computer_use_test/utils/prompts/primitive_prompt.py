...
{{
  "action": "click | press | drag | scroll | type",
  "x": 0, "y": 0,
  "end_x": 0, "end_y": 0,
  "scroll_amount": 0,
  "button": "left",
...
1. 생산 품목 선택 팝업이 보이면:
   Step A (탐색 — 첫 스텝에서 반드시 수행):
     ...
     - 목록 하단이 잘려 있으면 → 팝업 중앙에서 스크롤 다운. task_status="in_progress".
     - 목록 끝이면 → Step B로.
   ...
   Step B (결정): 모든 품목을 확인했으면 (더 이상 새 품목 없음 또는 목록 끝 도달):
     ...
     - 결정한 품목이 현재 화면에 없으면 (이전 스크롤에서 본 품목):
       팝업 중앙에서 스크롤 업 → task_status="in_progress".
...
공통 단계: 정책 카드 관리 화면 (좌: 슬롯, 우: 카드 목록)
  Step 1. 초기 슬롯/탭 파악
    ...
    - policy entry 직후 첫 정책 화면은 기본적으로 overview 화면으로 본다.
      군사/경제/외교/와일드카드/암흑 카드가 함께 보일 수 있다.
